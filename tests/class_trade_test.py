...
class TestTrade(unittest.TestCase):
    ...
    def test_scale_coordinates(self):
        test_data = Trade(file_path=settings.TEST_DATA_PATH, separator=';')
    ...
    def test_get_mean_velocity(self):
        ...
        pd.testing.assert_series_equal(result, expect, atol=1e-2)
    ...
    def test_com_distance_list(self):
        test_data = Trade(file_path=settings.TEST_DATA_PATH, separator=';')
        points = pd.read_csv(settings.TEST_PREDEFINED_POINTS_PATH, sep=';')
